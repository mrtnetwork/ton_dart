import 'package:test/test.dart';
import 'package:ton_dart/src/boc/boc.dart';
import 'package:ton_dart/src/models/models/transaction.dart';

void main() {
  _test();
}

void _test() {
  test('should parse transaction', () {
    // Source: https://explorer.toncoin.org/transaction?account=EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N&lt=22901965000001&hash=E9FB666FD65E2D70479C5A2C2EC412AD08D68FCDF57676B3BAA34AADA3C95DB8
    const boc =
        '';
    final cell = Cell.fromBase64(boc);
    final tx = TonTransaction.deserialize(cell.beginParse());

    final json = TonTransaction.fromJson(tx.toJson());
    final cell2 = beginCell().store(json).endCell();
    final decode = TonTransaction.deserialize(cell2.beginParse());
    expect(cell, cell2);
    expect(tx.toJson(), decode.toJson());
  });

  test('should parse transaction and hash', () {
    // Source: https://explorer.toncoin.org/transaction?account=EQCD39VS5jcptHL8vMjEXrzGaRcCVYto7HUn4bpAOg8xqB2N&lt=22901965000001&hash=E9FB666FD65E2D70479C5A2C2EC412AD08D68FCDF57676B3BAA34AADA3C95DB8
    const boc =
        '';
    final cell = Cell.fromBase64(boc);
    final tx = TonTransaction.deserialize(cell.beginParse());
    expect(tx.hash(), cell.hash());
    final json = TonTransaction.fromJson(tx.toJson());
    final cell2 = beginCell().store(json).endCell();
    final decode = TonTransaction.deserialize(cell2.beginParse());
    expect(cell, cell2);
    expect(tx.toJson(), decode.toJson());
  });
  test('should parse tick/tock transactions', () {
    // Source: https://explorer.toncoin.org/transaction?account=Ef80UXx731GHxVr0-LYf3DIViMerdo3uJLAG3ykQZFjXz2kW&lt=23019612000003&hash=36c7dbbb0cc6bf250e1265064c6c85751f8c838bea7caa58a9e62db03ea5d004
    const boc =
        '';
    final cell = Cell.fromBase64(boc);
    final tx = TonTransaction.deserialize(cell.beginParse());
    final json = TonTransaction.fromJson(tx.toJson());
    final cell2 = beginCell().store(json).endCell();
    final decode = TonTransaction.deserialize(cell2.beginParse());

    expect(cell, cell2);
    expect(tx.toJson(), decode.toJson());
  });
  test('should parse state init transaction', () {
    const boc =
        '';
    final cell = Cell.fromBase64(boc);
    final tx = TonTransaction.deserialize(cell.beginParse());
    final json = TonTransaction.fromJson(tx.toJson());
    final cell2 = beginCell().store(json).endCell();
    final decode = TonTransaction.deserialize(cell2.beginParse());

    expect(cell, cell2);
    expect(tx.toJson(), decode.toJson());
  });
  test('should parse messages with external addresses', () {
    // Source: https://explorer.toncoin.org/transaction?account=-1:a491d63f07ba7eefb4cb9f685484ce9089d5abaec97c15858222f04ca592a9ac&lt=22926061000001&hash=87572e72183eaa8be8dc0fb55921b0f6fbc02fa9e271ea7e2b21dcd76de3c26f
    const boc =
        '';
    final cell = Cell.fromBase64(boc);
    final tx = TonTransaction.deserialize(cell.beginParse());
    final json = TonTransaction.fromJson(tx.toJson());
    final cell2 = beginCell().store(json).endCell();
    final decode = TonTransaction.deserialize(cell2.beginParse());
    expect(cell, cell2);
    expect(tx.toJson(), decode.toJson());
  });
}
