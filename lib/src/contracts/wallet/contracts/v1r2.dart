...
class WalletV1R2 extends WalletContract {
  @override
  ...
  const WalletV1R2._({required this.state, required this.address});

  factory WalletV1R2({required int workChain, required List<int> publicKey}) {
    final state = VersionedWalletUtils.buildState(
        publicKey: publicKey, type: WalletVersion.v1R2);
    return WalletV1R2._(
        state: state,
  ...
  static Future<WalletV1R2> fromAddress(
    ...
    final state = VersionedWalletUtils.buildFromAddress(
        address: address, stateData: data.data, type: WalletVersion.v1R2);
    return WalletV1R2._(state: state.item1, address: address);
  }

  @override
  WalletVersion get type => WalletVersion.v1R2;
