...
class HighloadWalletV3 extends HighloadWallets<HighloadWalletV3State>
  ...
  Cell createInternalTransferBody({
    required List<OutAction> acctions,
  ...
  static Message createExternalMessage({
    ...
    final extMessage = Message(
      init: state,
      info: CommonMessageInfoExternalIn(dest: account, importFee: BigInt.zero),
      body: body,
    );
    return extMessage;
  }
  ...
  Future<String> sendBatchTransaction({
    ...
    final extMessage = createAndSignExternalMessage(
      signer: signer,
      message: packedAction(messages: messages, value: value, queryId: queryId),
      queryId: queryId,
      createAt: createAt,
      mode: mode,
    ...
    if (onEstimateFee != null) {
      await onEstimateFee(extMessage);
    }
    if (action.isBroadcast) return sendMessage(rpc: rpc, exMessage: extMessage);
