...
mixin ContractProvider {
  ...
  static Future<AccountStateResponse> getStaticState({
    ...
    if (rpc.isTonCenter) {
      final state = await rpc.request(
        TonCenterGetAddressInformation(address.toString()),
      );
    ...
    try {
      final state = await rpc.request(
        TonApiGetBlockchainRawAccount(address.toString()),
      );
      return AccountStateResponse(
        balance: state.balance,
        code: TonHelper.tryToCell(state.code),
